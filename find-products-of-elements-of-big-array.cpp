/*
The big_nums array is all power of 2. So we can represent it with the power only.
Once converted, the multiplication will become addition. We will then need to sum the value from from = query[0] to to = query[1].
2^i * 2^j = 2^(i + j)

Then, can use prefix sum
*/
